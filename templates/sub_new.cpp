...
#py if interface.tag == 'msg':
    ...
    {
        auto cb = [=](const `interface.cpp_type`::SharedPtr msg) { ros_callback__`interface.cpp_type_normalized`(msg, subscriberProxy); };
        rclcpp::QoS qos = 10;
        subscriberProxy->subscription = node->create_subscription<`interface.cpp_type`>(in->topicName, qos, cb);
    }
#py endif
#py endfor
