...
#py if interface.tag == 'srv':
    else if(serviceServerProxy->serviceType == "`interface.full_name`")
    {
        auto srv = boost::any_cast< std::shared_ptr< rclcpp::Service<`interface.cpp_type`> > >(serviceServerProxy->service);
        srv = nullptr;
    }
#py endif
#py endfor
