...
#py if interface.tag == 'srv':
    else if(clientProxy->serviceType == "`interface.full_name`")
    {
        ...
        read__`interface.request.cpp_type_normalized`(p->stackID, req.get(), &(clientProxy->rd_opt));
        auto result = cli->async_send_request(req);
        if(rclcpp::spin_until_future_complete(node, result) ==
                rclcpp::executor::FutureReturnCode::SUCCESS)
        {
            auto resp = result.get();
            write__`interface.response.cpp_type_normalized`(*resp, p->stackID, &(clientProxy->wr_opt));
        }
        else
        {
